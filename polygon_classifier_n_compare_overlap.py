...
st.caption("Upload 2 KML/KMZ → perbandingan → optional polygon spatial analysis")

with st.sidebar:
    st.header("⚙️ Pengaturan")
    threshold_m = st.number_input("Threshold jarak overlap (meter)", min_value=1, max_value=10_000, value=5, step=1)
    ...
    for i in range(1, 6):
        ...
        with st.expander(f"Polygon {i}{required_tag}", expanded=(i==1)):
            ...
            if pfile:
                ...
                polygon_files.append(pfile)
                polygon_rules[f"Polygon {i} ({pname})"] = prule
...
if st.button("🚀 PROSES", type="primary", use_container_width=True):
    ...
    if use_polygon and not polygon_files:
    ...
    with st.spinner("Memproses..."):
        try:
                ...
                for slot in polygon_slots:
                    ...
                    for df in [df_all, df_overlap, df_file_a_only, df_file_b_only]:
                        df[col_name] = df.apply(
                            lambda r: 'Dalam' if union_poly.contains(Point(r['Longitude'], r['Latitude'])) else 'Luar',
                            axis=1
                        )
            ...
            with tabs[1]:
                st.dataframe(df_overlap, use_container_width=True, height=400)
            with tabs[2]:
                st.dataframe(df_file_a_only, use_container_width=True, height=400) if len(df_file_a_only) > 0 else st.info("Kosong")
            with tabs[3]:
                st.dataframe(df_file_b_only, use_container_width=True, height=400) if len(df_file_b_only) > 0 else st.info("Kosong")
            
            if use_polygon:
                with tabs[4]:
                    st.dataframe(df_lolos_spasial, use_container_width=True, height=400) if len(df_lolos_spasial) > 0 else st.info("Kosong")
